    ...
    override fun transform(
        ...
        if (block.fields == null || block.fields!!.size != 2) {
            throw MalformedBlockException(
                block.type,
                "should have exactly two <FIELDS> defined: MONTH and DAY"
            )
        }

        val monthField =
            block.fields!!.find { it.name == "MONTH" }
                ?: throw MalformedBlockException(
                    block.type,
                    "should have <field name=\"MONTH\"> defined"
                )

        val dayField =
            block.fields!!.find { it.name == "DAY" }
                ?: throw MalformedBlockException(
                    block.type,
                    "should have <field name=\"DAY\"> defined"
                )
        ...
        val totalDaysValue =
            PortValueBuilder.buildFromDecimal(DayOfYearStamp::class.java, totalDays.toBigDecimal())
