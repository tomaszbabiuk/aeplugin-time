    ...
    override fun transform(
        ...
        if (block.fields == null || block.fields!!.size != 3) {
            throw MalformedBlockException(
                block.type,
                "should have exactly three <FIELDS> defined: HOUR, MINUTE and SECOND"
            )
        }

        val hourField =
            block.fields!!.find { it.name == "HOUR" }
                ?: throw MalformedBlockException(
                    block.type,
                    "should have <field name=\"HOUR\"> defined"
                )

        val minuteField =
            block.fields!!.find { it.name == "MINUTE" }
                ?: throw MalformedBlockException(
                    block.type,
                    "should have <field name=\"MINUTE\"> defined"
                )

        val secondField =
            block.fields!!.find { it.name == "SECOND" }
                ?: throw MalformedBlockException(
                    block.type,
                    "should have <field name=\"SECOND\"> defined"
                )
        ...
        val totalSecondsValue =
            PortValueBuilder.buildFromDecimal(
                TimeStamp::class.java,
                totalMillisecondsRaw.toBigDecimal()
            ) as TimeStamp
